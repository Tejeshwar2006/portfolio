...
export class ClockGrid {
  constructor(canvas, options = {}) {
    ...
    this.config = {
      ...
      lowPower: options.lowPower || false, // Reduces GPU cost (glow/shadows) — used on mobile
      ...options
    };

    // Cap canvas resolution on low-power / coarse-pointer devices
    this.isCoarsePointer = window.matchMedia('(pointer: coarse)').matches;
    this.maxDpr = this.isCoarsePointer ? 1.5 : 3;
  ...
  render() {
    ...
    for (let i = 0; i < numClocks; i++) {
      ...
      if (!this.config.lowPower && theme.handGlow && theme.handGlow !== 'rgba(255, 255, 255, 0)') {
      ...
      if (theme.centerDot || theme.accentDot) {
        ...
        if (!this.config.lowPower && dotProxFactor > 0.05) {
